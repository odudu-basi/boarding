...
// WEB GRADIENTS: No LinearGradient needed — wrapWithGradient uses CSS linear-gradient on web

// WEB ICONS: Helper function to get web icon component from react-icons
function getWebIconComponent(library: string, iconName: string): any {
  ...
  const libraryMap: Record<string, any> = {
    ...
    lucide: FiIcons,
    material: MdIcons,
    'material-community': MdIcons,
    ionicons: IoIcons,
    fontawesome: FaIcons,
    'sf-symbols': IoIcons,
  };

  const iconSet = libraryMap[library.toLowerCase()];
  if (!iconSet) return null;

  // Convert icon name to PascalCase with prefix
  // e.g., "home" -> "FiHome", "settings" -> "MdSettings"
  const prefixMap: Record<string, string> = {
    feather: 'Fi',
    lucide: 'Fi',
    material: 'Md',
    'material-community': 'Md',
    ionicons: 'Io',
    fontawesome: 'Fa',
    'sf-symbols': 'Io',
  };

  const prefix = prefixMap[library.toLowerCase()] || 'Fi';
  ...
  const componentName = prefix + pascalName;
  return iconSet[componentName];
}

// Try to import vector icons — optional peer dependency (for mobile)
...
try {
  ...
  IconSets = {
    ...
    'sf-symbols': icons.Ionicons,
  };
} catch {
  // Not available on web — will use react-icons instead
}
...
const RenderNode: React.FC<RenderNodeProps> = ({ element, toggledIds, groupSelections, onAction, variables, onSetVariable, assets }) => {
  ...
  switch (element.type) {
    ...
    case 'icon': {
      ...
      // WEB ICONS: Try react-icons first (web compatibility)
      const WebIconComponent = getWebIconComponent(library, iconName);
      if (WebIconComponent) {
        return (
          <View style={[style, { alignItems: 'center', justifyContent: 'center' }]}>
            <WebIconComponent size={iconSize} color={iconColor} />
          </View>
        );
      }

      // Fallback to @expo/vector-icons (mobile only)
      ...
      // Final fallback — render icon name as text placeholder
      return (
        <View
          style={[
            style,
...
function convertStyle(style: ElementStyle | Record<string, any>): ViewStyle & TextStyle {
  ...
  // backgroundGradient is handled by wrapWithGradient at the component level (CSS on web).

  return rnStyle;
}

// ─── Gradient Wrapper (CSS Web Implementation) ───

function coordsToAngle(start: { x: number; y: number }, end: { x: number; y: number }): number {
...
function wrapWithGradient(
  ...
  if (!gradient || !gradient.colors?.length) return content;

  // Handle both { color, position } objects and plain color strings
  const colorStops = gradient.colors.map((c: any, i: number, arr: any[]) => {
  ...
  if (gradientType === 'radial') {
    backgroundImage = `radial-gradient(circle, ${colorStops.join(', ')})`;
  } else {
    let angle: number;
    if (gradient.start && gradient.end) {
      ...
      angle = coordsToAngle(s, e);
    } else {
      angle = gradient.angle ?? 180;
    }
    backgroundImage = `linear-gradient(${angle}deg, ${colorStops.join(', ')})`;
  }
