...
export default function AnalyticsClient({ initialData }: { initialData: AnalyticsInitialData }) {
  const router = useRouter()
  const supabase = createClient()
  const [loading, setLoading] = useState(false)
  const [organization, setOrganization] = useState<any>(initialData.organization)
  const [events, setEvents] = useState<any[]>(initialData.initialEvents)
  const [revenueCatEvents, setRevenueCatEvents] = useState<any[]>(initialData.initialRevenueCatEvents)
  ...
  const [initialProjectId] = useState(initialData.projectId)
  ...
  useEffect(() => {
    ...
    initialData.initialConfigs?.forEach((config: any) => {
      configMap.set(config.id, config)
    })
    setFlowConfigs(configMap)

    // Calculate initial stats
    computeStats(initialData.initialEvents, initialData.initialCount, initialData.initialRevenueCatEvents, configMap)

    trackAnalyticsPageViewed(initialData.projectId || initialData.organization.id, initialData.initialEvents.length > 0)
  }, [])

  // Reload data when time range changes (after initial mount)
  ...
  const computeStats = (eventsData: any[], count: number, revData: any[], configMap: Map<string, any>) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const projectCookie = initialProjectId || document.cookie
        .split('; ')
      ...
      computeStats(eventsData || [], count || 0, revData || [], flowConfigs)
